...
if __name__ == "__main__":
    ...
    for idx, i in enumerate(translation_suite):
        ...
        if idx == 0:
            ax.bar(np.arange(len(x1)), x1, color='gray', edgecolor='black', 
                    label="Timely", zorder=3)
            ax.bar(np.arange(len(x1)), x2, color='black', edgecolor='black', 
                    label="Late", zorder=2)
        else:
            ax.bar(np.arange(len(x1)), x1, color='gray', edgecolor='black', 
                    zorder=3)
        ...
        ax.set_xticklabels(torder, rotation=25)
        ...
        ax.set_xlabel("{} {}".format(elem[idx], i))
